...
// Tornamos a função global para o botão do HTML conseguir chamá-la
function checarPlanilha() {
    ...
    jQuery.ajax({
        ...
        success: function(data) {
            ...
            jQuery("#last-sync").html(horaFormatada);
            ...
            if (senhaNormalFinal !== memoriaNormal) {
                jQuery("#senhaNormalNumero").html(senhaNormalFinal);
            ...
            if (senhaPrefFinal !== memoriaPref) {
                jQuery("#senhaPrefNumero").html(senhaPrefFinal);
                if (memoriaPref !== "") tocarSom();
                memoriaPref = senhaPrefFinal;
            }
        },
        error: function() {
            jQuery("#last-sync").html("<span style='color:red'>Erro de Conexão</span>");
        }
    });
}
...
jQuery(document).ready(function($) {
    // Mantém a verificação automática de 15s
